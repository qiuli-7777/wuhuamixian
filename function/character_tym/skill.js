...
const skills = {
    ...
    ql_wanbu: {
		...
		getColorValue(player, color) {
            const cards = player.getCards("hej", card => get.color(card) == color);
            if (!cards.length) return 0;
            const totalValue = cards.reduce((sum, card) => sum + get.value(card, player), 0);
            const avg = totalValue / cards.length;
            let score = (6 - avg) * cards.length;   // 基础分：平均价值越低，分越高
            // 判定区有该颜色牌（大概率是负面延时牌），强烈倾向于重铸
            const judgeCards = player.getCards("j", card => get.color(card) == color);
            if (judgeCards.length) score += 10;
            // 有防具或+1马属于该颜色，降低重铸意愿
            const badEquips = player.getCards("e", card => {
                if (get.color(card) != color) return false;
                const sub = get.info(card).subtype;
                return sub == "equip2" || sub == "equip3";  // 防具和+1马
            });
            if (badEquips.length) score -= 5 * badEquips.length;
            return score;
        },
		chooseButton: {
			dialog(event, player) {
			...
			filter(button) {
				...
				if (button.link == "black" && !player.countCards("hej", card => get.color(card) == "black")) {
					return false;
				}
				if (button.link == "red" && !player.countCards("hej", card => get.color(card) == "red")) {
					return false;
				}
				return true;
			},
			check(button) {
			    ...
			    return get.info("ql_wanbu").getColorValue(player, button.link);
			},
			backup(links, player) {
				return {
				    filterCard: (card) => {
		...
		ai: {
		    ...
		    result: {
		        player(player) {
		            const info = get.info("ql_wanbu");
		            return (info.getColorValue(player, "black") > 0 || info.getColorValue(player, "red") > 0) ? 1 : 0;
		        },
		    },
		},
    },
    ql_meixin: {
        ...
        async content(event, trigger, player) {
            if(trigger.getl(player).cards2.length > 0) {
                ...
                .set("num", trigger.getl(player).cards2.length)
                .forResult();
                if(result?.bool) {
    ...
    ql_jishou: {
        ...
        async content(event, trigger, player) {
            ...
            if(cards.length) {
                ...
                if(2-player.countMark(name) > 0) {
                    await player.showCards(player.getCards("h"));
                    await player.discardPlayerCard(player, "hej", 2-player.countMark(name), true);
                    player.addMark(name);
    ...
    ql_lianci: {
        ...
        async content(event, trigger, player) {
            ...
            if(!player.getStorage(name + "_discard").includes(type1)) {
                ...
                .filter(evt => evt.card && get.type2(evt.card) == type1)
                .step(async (event, trigger, player) => {
                    await player.draw();
                })
            }
			...
			backup(links, player) {
				return {
					...
					filterCard(card, player) {
						return player.countCards("h") > 5 && lib.filter.cardDiscardable(card, player, "ql_luyou");
					},
					position: "h",
					selectCard() {
						const player = get.player(),
							num = player.countCards("h") - 5;
						if (num > 0) {
							return num;
						}
						return -1;
    ...
    ql_fengling: {
        ...
        async cost(event, trigger, player) {
            if(trigger.name == "phaseJieshu") {
                event.result = await player.chooseBool(player.countCards("h") < player.maxHp ? `是否翻面并摸${get.translation(player.maxHp)}张牌` : "是否移动场上一张牌").forResult();
            } else {
                event.result = {
                    bool: true,
                }
    ...
    ql_yaomei: {
        ...
        async content(event, trigger, player) {
            let num = 1, i = 1;
            while(event.targets.length > i) {
                const result = await event.targets[i].chooseBool("是否受到一点伤害").forResult();
                if(result.bool) {
                    await event.targets[i].damage();
                } else {
                    num++;
                }
                i++;
                //game.log("当前i="+i+"总数为"+event.targets.length);
            }
