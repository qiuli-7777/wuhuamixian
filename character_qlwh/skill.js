...
const skills = {
	...
	ql_baojian: {
		...
		intro: {
			content(storage, player, skill) {
				if (!storage) {
					return "其他角色于摸牌阶段外获得牌后，选择一项：①展示且本回合不能对你使用这些牌；②令你回复一点体力（若满血改为摸牌）。若本轮没有角色以此法展示过该点数的牌，你防止你下一次受到的伤害并摸一张牌。";
				}
				return "其他角色于弃牌阶段外失去牌置入弃牌堆后，当前回合角色对你使用的下一张牌若不与这些牌点数相同则取消之并弃置一张牌。";
			},
		},
		trigger: {
			global: ["phaseBefore", "roundStart", "gainAfter", "loseAfter", "loseAsyncAfter", "cardsDiscardAfter", "equipAfter"],
			player: "enterGame",
		},
		firstDo: true,
		frequent: true,
				...
				if(game.roundNumber % 2 != 0) {
					...
					if(event.name == "loseAsync") {
						return game.filterPlayer(current => {
								return event?.getg?.(current)?.length && current.countCards("h", card => event?.getg?.(current)?.includes(card));
							})
							.sortBySeat();
					}
					return false;
					...
					return game
                        .filterPlayer(function (target) {
                            if (event.name != "cardsDiscard") {
                                ...
                                if (!event.cards.filterInD("d").length) {
                                    return false;
                                }
		...
		filter(event, player, name, target) {
			if(name.includes("After")) {
				...
				let evtg = event.getParent("phaseDraw");
                let evtl = event.getParent("phaseDiscard");
                if ((evtg && evtg.player == target) || (evtl && evtl.player == target)) {
                    return false;
                }
				return true;
			} else {
				return event.name != "phase" || event?._roundStart || game.phaseNumber == 0;
			}
		},
		logTarget(event, player, name, target) {
			if(name.includes("After")) {
				return game.roundNumber % 2 == 0 ? _status.currentPhase : target;
			}
			return player;
		},
		async content(event, trigger, player) {
			if(event.triggername.includes("After")) {
				if(game.roundNumber % 2 != 0) {
					...
					.set("choiceList", [`展示${get.translation(cards)}且本回合不能对${get.translation(player)}使用这些牌`, `令${get.translation(player)}${player.getDamagedHp() == 0 ? `摸一张牌` : `回复一点体力`}`])
					.set("choice", get.rand(0, 1))
					.set("forced", true)
					.forResult();
                    if (cards.length && result.index % 2 == 0) {
                        await target.showCards(cards);
						target.addTempSkill(event.name + "_unuse");
						target.addGaintag(cards, event.name);
						player.addTempSkill(event.name + "_showed", { global: "roundEnd" });
						const showCards = cards.map(card => get.number(card)).unique().filter(num => !player.getStorage(event.name + "_showed").includes(num));
						player.markAuto(event.name + "_showed", showCards);
						player.addSkill(event.name + "_defend");
						player.addMark(event.name + "_defend", showCards.length, false);
						player.draw(showCards.length);
                    }
					if(result.index % 2 != 0) {
						await player[player.getDamagedHp() == 0 ? "draw" : "recover"](1);
					}
				} else {
					const target = event.indexedData;
                    let cards;
                    if (trigger.name != "cardsDiscard") {
                        cards = trigger.getd(target, "cards2");
                    } else {
                        cards = trigger.cards.filterInD("d");
                    }
                    if (cards?.length) {
                        _status.currentPhase.addTempSkill(event.name + "_cancel");
						_status.currentPhase.setStorage(event.name + "_cancel", cards.map(card => get.number(card)).unique(), true);
                    }
				}
				return;
			}
			if(trigger._roundStart && game.phaseNumber > 1) {
			...
			const gainList = list.filter(element => !player.getCards("h").map(card => get.number(card)).unique().includes(element));
			gainList.forEach(item => {
				const card = get.cardPile(card => get.number(card) == item);
				if(card) {
					cardList.add(card);
				}
			});
		...
		subSkill: {
			defend: {
				...
				filter(event, player) {
					return player.countMark("ql_baojian_defend");
				},
				async content(event, trigger, player) {
					trigger.cancel();
			...
			unuse: {
				...
				onremove: true,
				intro: {
					content: "被标记的牌不能对铜壶滴漏使用",
				},
				mod: {
					playerEnabled(card, player, target) {
						if(get.translation(target).indexOf("滴漏") >= 0 && card?.cards?.some(cardx => cardx.hasGaintag("ql_baojian"))) {
							return false;
						}
					},
				},
			...
			cancel: {
				...
				filter(event, player) {
					return player.getStorage("ql_baojian_cancel").length;
				},
				async content(event, trigger, player) {
					if(player.getStorage(event.name).every(num => get.number(trigger.card) == num)) {
						return;
					}
	...
	ql_shuiying: {
		...
		filter(event, player) {
			return player.getStorage("ql_shuiying_used").length < 4;
		},
		filterCard: (card) => {
			const player = get.player();
			return !player.getStorage("ql_shuiying_used").includes(get.suit(card));
		},
		position: "he",
		filterTarget: true,
		lose: false,
		discard: false,
		async content(event, trigger, player) {
			const { name, cards: [card], targets: [target] } = event;
			if(player.storage[name]) {
				player.addTempSkill(name + "_used");
				player.markAuto(name + "_used", get.suit(card))
			}
			await player.recast(card);
			if(player.getFriends(true).includes(target)) {
				await target.gainMaxHp();
				await target.recover();
				await game.doAsyncInOrder([player, target], async current => {
					const cardGain = get.cardPile(cardx => get.suit(cardx) == get.suit(card));
					if(cardGain) {
						await current.gain(cardGain, "gain2");
					}
				});
				let result = await target.chooseNumbers(`${get.translation(name)}:获得一张指定点数的牌`, [{
					min: 1,
					max: 13,
				}])
				.set("processAI", () => Math.random())
				.forResult();
				if(result?.bool) {
					const cardGain2 = get.cardPile(card => get.number(card) == result.numbers);
					if(cardGain2) {
						await target.gain(cardGain2, "gain2");
					}
				}
				target.addSkill(name + "_cause")
				target.addMark(name + "_cause", 1, false);
				if(target.getSeatNum() != 1) {
					result = await target.chooseBool(`${get.translation(name)}:是否与${get.translation(target.getPrevious())}交换座次`).forResult();
					if(result?.bool) {
						game.broadcastAll((x, y) => {
							game.swapSeat(x, y);
				...
				await target.damage();
				await player.gain(target.getCards("he", cardx => get.suit(cardx) == get.suit(card)), "gain2");
				target.addSkill([name + "_fragile", name + "_receive"]);
				target.addMark(name + "_receive", 1, false);
				if(target.getSeatNum() <= game.players.length) {
					game.broadcastAll((x, y) => {
						game.swapSeat(x, y);
	...
	ql_louke: {
		...
		filter(event, player, name) {
			if(name == "phaseAfter") {
				game.broadcastAll(() => {
					...
					const overlay = document.querySelector('.ql_louke_overlay');
					if (overlay) overlay.remove();
				});
				return game.hasPlayer(current => {
					return player.hasHistory("useSkill", evt => evt.skill == "ql_shuiying" && evt?.targets?.includes(current));
				});
			}
			return true;
		},
		...
		async content(event, trigger, player) {
			if(event.triggername == "phaseAfter") {
				game.players.forEach(current => {
					if(player.getFriends(true).includes(current)) {
						switch(player.getHistory("useSkill", evt => evt.skill == "ql_shuiying" && evt?.targets?.includes(current)).length) {
							case 4:
							case 3:
								const phases = [];
								current.addSkill(event.name + "_effect");
								current.when({ player: "phaseEnd" })
								.filter((evt, player) => {
									phases.add(evt);
									return phases.length >= 2;
								})
								.step(async (event, trigger, player) => {
									player.removeSkill("ql_louke_effect")
								})
							case 2:
								current.recoverTo(current.maxHp);
							case 1:
								current.addSkill(event.name + "_effect2");
							default:
								return;
						}
					} else {
						switch(player.getHistory("useSkill", evt => evt.skill == "ql_shuiying" && evt?.targets?.includes(current)).length) {
							case 4:
							case 3:
								current.loseHp(current.maxHp);
							case 2:
								current.turnOver(true);
							case 1:
								current.addSkill(event.name + "_basic");
							default:
								return;
						}
					}
			...
			if(!player.storage[event.name]) {
				game.broadcastAll((bg, bgm) => {
					...
					dialog.muted = false;
					dialog.setAttribute("src", `${lib.assetURL}extension/五花米线/video/铜壶滴漏.mp4`);
					...
					overlay.className = 'ql_louke_overlay';
					...
					document.documentElement.style.filter = "grayscale(30%) invert(20%)";
					document.documentElement.style.transform = "scale(1.05)";
				}, `ext:五花米线/skin/background/铜壶滴漏.png`, `ext:五花米线/audio/background/铜壶滴漏.mp3`)
				player.storage[event.name] = true;
				player.storage.ql_shuiying = true;
				let gains = [];
				while (true) {
				...
				player.addTempSkill(event.name + "_use","phaseAfter")
				player.when({ player: "phaseAfter" })
				.step(async (event, trigger, player) => {
					player.storage.ql_shuiying = false;
				})
		...
		subSkill: {
			effect: {
				...
				trigger: {
					player: ["damageBegin4", "loseHpBefore", "loseMaxHpBefor", "dieBegin"],
				},
				filter(event, player) {
					return event.name != "die" || player.hp > 0;
				},
				async content(event, trigger, player) {
					trigger.cancel();
					trigger.num = 0;
			...
			use: {
				...
				async content(event, trigger, player) {
					...
					trigger.card.ql_louku_unequip = true;
				},
				ai: {
					unequip: true,
				},
	...
	ql_yunfeng: {
		...
		filter(event, player) {
			return !player.hasHistory("useCard", evt => !game.players.concat(game.dead).some(current => current != player && evt?.targets?.includes(current)))
		},
		async cost(event, trigger, player) {
			event.result = await player.chooseTarget({
				prompt: "令至多两名其他角色只能使用指定你为目标的牌",
		...
		async content(event, trigger, player) {
			...
			if(player.hp > player.getDamagedHp) {
				await player.loseHp();
				player.addTempSkill(name + "_recover", { player: "phaseBegin" });
			}
			player.line(targets);
			targets.forEach(target => {
				player.markAuto(name + "debuff", target);
				target.addTempSkill(name + "_debuff", { player: "phaseEnd" });
			})
		},
		subSkill: {
			recover: {
			...
			debuff: {
				mod: {
					playerEnabled(card, player, target) {
						if (!player.getStorage("ql_yunfeng_debuff").includes(target)) {
							return false;
						}
					},
					cardEnabled(card, player) {
	...
	ql_feipu: {
		...
		filter(event, player) {
			if(event.name == "damage") {
				return event.source && get.distance(event.source, player) <= 1 && player.hasCard(card => {
					const vcard = get.autoViewAs({ name: "sha" }, [card]);
					return lib.filter.targetEnabled2(vcard, player, event.source);
				});
			}
			return _status.currentPhase != event.player && get.distance(event.player, player) <= 1 && event.player != player;
		},
		async content(event, trigger, player) {
			if(trigger.name == "damage") {
				await player.draw();
				const next = await player.chooseToUse();
				next.set("filterCard", (card, player) => {
					return lib.filter.filterCard.apply(this, arguments);
				});
				next.set("openskilldialog", `飞瀑，是否将一张牌当做【杀】对${get.translation(trigger.source)}使用`);
				...
				next.set("filterTarget", (card, player, target) => {
					...
					return lib.filter.targetEnabled2(card, player, targetx);
				});
				next.set("addCount", false);
				next.set("targetx", trigger.source);
				return;
			}
	...
	ql_tianxuan: {
		...
		filter(event, player, name) {
			...
			return game.hasGlobalHistory("everything", evt => evt.name == "addPlayer");
		},
		forced: true,
		onRound(event) {
			return !event.ql_tianxuan_phase;
		},
		async content(event, trigger, player) {
			const next = player.insertPhase();
			next._noTurnOver = true;
			if (event.triggername != "phaseEnd") {
				player.setStorage("ql_tianxuan_first", true);
				if (!trigger._finished) {
					/**/trigger.finish();
					trigger._finished = true;
					trigger._triggered = 5;
					if (!lib.onround.includes(lib.skill.ql_tianxuan.onRound)) {
						lib.onround.push(lib.skill.ql_tianxuan.onRound);
					}
					const evt = trigger.player.insertPhase();
					evt.ql_tianxuan_phase = true;
					...
					evt.pushHandler("ql_tianxuan_phase", (event, option) => {
			...
			player
				...
				.filter(evt => evt == next)
				.then(() => player.addTempSkill("ql_tianxuan_effect"));
		},
		group: ["ql_tianxuan_summoner"],
		subSkill: {
			effect: {
				audio: "ql_tianxuan",
				charlotte: true,
				trigger: {
					player: ["phaseZhunbeiBegin", "phaseJieshuBegin", "phaseAnyCancelled", "phaseAnySkipped"],
				},
				filter(event, player, name) {
					...
					if (event.triggername.endsWith("Begin")) {
						...
						event.result = await player
							.chooseBool({
								...
								prompt2: `执行一个额外的${get.translation(name)}`,
								choice: !["phaseDiscard", "phaseJudge"].includes(name),
							})
							.forResult();
					}
				},
				async content(event, trigger, player) {
					if (event.triggername.endsWith("Begin")) {
						...
						evt.phaseList.splice(evt.num + 1, 0, `${trigger.name}|${event.name}`);
					}
				}
			},
			summoner: {
				...
				forced: true,
				filter(event, player) {
					if (!game.hasPlayer(target => target._source == player)) {
						return false;
					}
					if (event.name == "useCard") {
						return event.targets?.some(target => target.isIn()) && ["basic", "trick"].includes(get.type(event.card));
					}
					return true;
				},
				logTarget: (event, player) => game.filterPlayer(target => target._source == player),
				async content(event, trigger, player) {
					const { targets } = event;
					await game.doAsyncInOrder(targets, async target => {
						...
						if (trigger.name == "draw") {
							...
							result = await target
								.chooseUseTarget({
									card: get.autoViewAs({ name: get.name(card), isCard: true, nature: get.nature(card) }),
									nodistance: true,
								})
								.set("targets", targets)
								.forResult();
						...
						if (result?.bool) {
							target.addSkill(`ql_tianxuan_damage`);
							target.addMark(`ql_tianxuan_damage`, 1, false);
						}
					})
				}
			},
			damage: {
				...
				filter(event, player) {
					return player.hasMark("ql_tianxuan_damage");
				},
				async content(event, trigger, player) {
					trigger.num += player.countMark(event.name);
				}
	...
	ql_tongshuo: {
		...
		async content(event, trigger, player) {
			...
			game.log(trigger.card, "数值+1");
			trigger.baseDamage++;
			await player.draw();
			const targets = game.filterPlayer(target => target != player && target.countCards("h") > 1);
			if (targets.length) {
				const result = await player
					.chooseTarget({
						prompt: "同烁：令一名其他角色将手牌弃至一张且受到召唤物造成的伤害+1至其回合结束",
						filterTarget(card, player, target) {
				...
				if (result.bool) {
					...
					await target
						.chooseToDiscard({
							position: "h",
							selectCard: target.countCards("h") - 1,
							forced: true,
						});
					target.addTempSkill(event.name + "_damage", { player: "phaseAfter" });
					const storage = target.getStorage(event.name + "_damage", {});
					storage[player.playerid] ??= 0;
					storage[player.playerid]++;
					target.setStorage(event.name + "_damage", storage, true);
				}
			}
		},
		subSkill: {
			record: {
				...
				async content(event, trigger, player) {
					player.addMark("ql_tongshuo", 1, false);
					player.addTempSkill("ql_tongshuo_clear", "roundStart");
				}
			},
			clear: {
				charlotte: true,
				onremove(player, skill) {
					player.clearMark("ql_tongshuo", false);
				}
			},
			damage: {
				audio: "ql_tongshuo",
				...
				filter(event, player) {
					return player.getStorage("ql_tongshuo_damage", {})[event.source?._source?.playerid || ""] > 0;
				},
				async content(event, trigger, player) {
					trigger.num += player.getStorage(event.name, {})[trigger.source._source.playerid] || 0;
				},
				intro: {
	...
	ql_zhuyao: {
		...
		enable: "phaseUse",
		onChooseToUse(event) {
			if (!game.online && !event.ql_zhuyao) {
				event.set("ql_zhuyao", game.getGlobalHistory("everything", evt => {
					return evt.name == "gain" && evt.player._source != event.player;
				}).reduce((sum, evt) => sum + evt.cards.length, 0) >= 4);
			}
		},
		filter(event, player) {
			return event.ql_zhuyao;
		},
		async content(event, trigger, player) {
			game.broadcastAll((bg, bgm) => {
				if (_status.tempBackground != bg) {
			...
			}, `ext:五花米线/skin/background/天球仪.png`, `ext:五花米线/audio/background/天球仪.mp3`)
			let num = 2;
			while (num > 0) {
				num--;
				const result = await player
					.chooseTarget({
						prompt: "珠曜：请选择拟态恒星的上家",
						forced: true,
						ai(target) {
							return -get.attitude(get.player(), target) * Math.random();
						}
					})
					.forResult();
				if (result?.targets?.length) {
					const { targets: [target] } = result;
					player.line(target);
					await player.ql_addPlayer(target, "ql_nitaihengxing", null, true, { startCards: 4, noCheckResult: true });
				} else {
					break;
				}
			}
		...
		group: ["ql_zhuyao_die", "ql_zhuyao_buff"],
		subSkill: {
			die: {
				audio: "ql_zhuyao",
				forceDie: true,
				...
				charlotte: true,
				trigger: {
					global: ["dieBegin", "dying"],
				},
				forced: true,
				filter(event, player) {
					if(event.player != player) {
						return event.player._source == player && event.name == "die";
					}
					return true;
				},
				async content(event, trigger, player) {
					if(event.player != player) {
						if(game.hasPlayer(target => target.name == "ql_nitaihengxing" && target._source == player)) {
							const result = await player
								.chooseTarget({
									prompt: "珠曜：令一张『拟态恒星』死亡，回复体力至上限，然后获得牌名不同的基本牌各一张",
									forced: true,
									filterTarget(card, player, target) {
										return target.name == "ql_nitaihengxing" && target._source == player
									},
									ai(target) {
										return Math.random();
									}
						...
						const cards = [];
						while (true) {
							const card = get.cardPile(card => !names.includes(get.name(card)) && get.type(card) == "basic");
							if (card) {
								names.push(get.name(card));
								cards.push(card);
							} else {
								break;
							}
						...
						const result = await player.chooseTarget({
							prompt: "令一名其他角色不能造成伤害和回复体力且获得牌时改为你获得至你的回合开始。",
							filterTarget(card, player, target) {
								return target != player;
							},
							ai(target) {
						...
						.forResult();
						if(result.bool) {
							player.markAuto(`${event.name.slice(0, -4)}_buff`, result.targets);
						}
					}
				}
			},
			buff: {
				...
				forced: true,
				intro: {
					content: "$不能造成伤害和回复体力且获得牌时改为你获得",
				},
				trigger: {
					global: ["damageBegin1", "recoverBefore", "gainBefore"],
					player: "phaseAfter",
				},
				filter(event, player) {
					const storage = player.getStorage("ql_zhuyao_buff");
					if(event.name == "damage") {
						return storage.includes(event.source);
					}
					if(event.name != "phase") {
				...
				async content(event, trigger, player) {
					if(trigger.name != "phase") {
						...
						if(trigger.name == "gain") {
							...
							const cards2 = trigger.cards.filter(card => !cards.includes(card) && !get.position(card));
							if(cards2.length) {
								await player.gain(cards, "gain2");
							}
						}
	...
	ql_zhuxu: {
		mod: {
			cardEnabled(card, player) {
				...
				if (_status.currentPhase == player && [card, ...(card.cards || [])]?.containsSome(...hs)) {
					return false;
				}
			},
			cardSavable(card, player) {
		...
		enable: "phaseUse",
		filter(event, player) {
			return game.hasPlayer(target => target == player._source);
		},
		position: "he",
		filterCard: true,
		filterTarget(card, player, target) {
			return player._source == target;
		},
		lose: false,
		delay: false,
		discard: false,
	...
	ql_tansuo: {
		...
		async content(event, trigger, player) {
			const result = await player
				.chooseTarget({
					prompt: "坍缩：请选择一名要更换座次的角色，将自己移动到该角色的上家位置",
					filterTarget(card, player, target) {
					...
					ai(target) {
						const player = get.player();
						return get.damageEffect(target, player, player) + get.damageEffect(target.getPrevious(), player, player);
					}
				})
				.forResult();
			game.broadcastAll(
			...
			const targets = [player.getNext(), player.getPrevious()].unique().filter(target => ((target != player._source) && (target?._source != player._source)));
			if (targets.length) {
				player.line(targets);
				//game.players.concat(game.dead).forEach(target => target.addTempSkill(event.name + "_hook"));
				let num = 0;
				await game.doAsyncInOrder(targets, async target => {
					const next = target.damage(player._source.maxHp).set("notriggerDamage", true);
					await next;
					if (!target.hasHistory("damage", evt => evt == next && evt.num > 0)) {
						await target.loseHp(Math.max(player._source.maxHp, next.num));
					}
					num += next.num;
				});
				event.forceDie = true;
				event.forceOut = true;
				const source = player._source;
				await player.die();
				if (source?.isIn()) {
					await source.gainMaxHp(num);
					await source.recover(num);
			...
			hook: {
				hookTrigger: {
					block(event, player, triggername, skill) {
						if (event.name == "damage") {
							return event.notriggerDamage && event.source == player;
						} else if (event.name == "changeHp") {
							const evt = event.getParent();
							return evt.name == "damage" && evt?.notriggerDamage && evt.source == player;
						}
					}
				}
			},
		...
		async content(event, trigger, player) {
			...
			if (result.targets?.length) {
				...
				if (typeof resultx?.index == "number") {
					...
					const { index } = resultx;
					const callback = async (event, player) => {
						player.qltitou_source = event.player;
					};
					await player.ql_addPlayer(target, name, null, !!index, { identity: "ql_ying", noCheckResult: true, callback });
